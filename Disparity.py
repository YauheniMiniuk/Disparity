...
STRUCT_PATH = '! Копия Веса.xlsx'
RB_SHEET = 'РБ_Струкутура'
RF_SHEET = 'РФ_структура'

RB_PRICES_PATH = 'Average_prices-06-2023.xls'
RF_PRICES_PATH = 'rf_06.2023.xls'

PROD_RESULT_PATH = 'Диспаритет продовольственных товаров РБ-РФ.xlsx'
NOPROD_RESULT_PATH = 'Диспаритет непродовольственных товаров РБ-РФ.xlsx'

REGION_PAIRS = [('Гомельская', 'Брянская область'),
                ('Могилевская', 'Брянская область'),
                ('Могилевская', 'Смоленская область'),
                ('Витебская', 'Смоленская область'),
                ('Витебская', 'Псковкая область')]
...
def compareRF(list1, df2, colname2, colname2prices, perc, noProd=False):
    ...
    for i in range(len(list1)):
        ...
        for j in range(len(list2)):
            ...
            # score = get_cosine_similarity_new(name1Clean, clean_string(list2[j]))
            score = combine_cosine_levenshtein_jar_win(name1Clean, clean_string(list2[j]), 0.45, 0.5, 0.05)
            if score > min_dist:
                min_dist = score
                similar_index = j
        if min_dist >= perc:
...
def loadProdPricesRB(path=RB_PRICES_PATH, header=7):
...
def loadNotProdPricesRB(path=RB_PRICES_PATH, header=7):
...
def compare_rb_rf(list1, list2, perc, noProd = False):
...
class Category:
    ...
    def to_excel(self, filename, exchange_rate, region_pairs):
        ...
        for region_pair in region_pairs:
            columns.append(f"Диспаритет {region_pair[0]}-{region_pair[1]}") # add column name for disparity
        ...
        for group in self.groups:
            ...
            for product in group.products: # add products that are in the group list but not in the subgroup list
                ...
                for i in range(len(region_pairs)):
                    price_BYN = product.pricesBYN.get(region_pairs[i][0], None) # get the price in BYN for the region, or None if not available
                    price_RUB = product.pricesRUB.get(region_pairs[i][1], None) # get the price in RUB for the region, or None if not available
                    if price_RUB and price_BYN: # if both prices are available
                        ...
                        ws.cell(row=row_index, column= 3 + i).value = disparity # write disparity to the corresponding column
                    else: # if one or both prices are not available
                        ws.cell(row=row_index, column= 3 + i).value = "" # write "Н/Д" (not available) to the corresponding column
                row_index += 1 # increment row index
...
df2 = pd.read_excel(STRUCT_PATH, sheet_name=RB_SHEET, header=8)
...
wb = openpyxl.load_workbook(STRUCT_PATH)
ws = wb.get_sheet_by_name(RB_SHEET)
...
comparedProdRB = compareRB(prodRB, prodPricesRB, prodPricesRB.columns[0], 0.75)
...
rf = pd.read_excel(STRUCT_PATH, sheet_name=RF_SHEET, header=3)
...
prod_rb_rf = compare_rb_rf(prodRB, prodRF, 0.5)

noProd_rb_rf = compare_rb_rf(noProdRB, noProdRF, 0.5)

prodCategoryRB.to_excel(PROD_RESULT_PATH, 0.035618, REGION_PAIRS)
noProdCategoryRB.to_excel(NOPROD_RESULT_PATH, 0.035618, REGION_PAIRS)